...
사용자는 현재 핫스팟(월 11GB 한도)으로 인터넷을 사용 중이다.
작업을 진행하기 전, 해당 작업이 상당한 데이터를 소모할 것으로 예상되면 반드시 아래 순서를 따른다:
상당한 데이터는 한 번에 100MB 이상을 소모하는 작업을 말한다.
1. 예상 데이터 사용량 고지 (예: Docker 이미지 pull + Gradle 의존성 다운로드 = 약 X GB)
2. 데이터를 덜 쓰는 대안(B안)이 있으면 함께 제시
3. A안 / B안 각각의 예상 사용량과 trade-off 설명
4. 사용자의 결정을 듣고 진행

> 이 파일 상단에 "와이파이 사용 중" 문구가 있으면 데이터 걱정 없이 진행해도 된다.
