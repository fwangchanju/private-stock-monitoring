...
- **DB**: MariaDB 10.6
- **Frontend**: React + Vite + TypeScript (미구현)
- **인프라**: Docker, Nginx (미구성), GHCR
...
- **1번 서버**: 앱 운영 서버. psmsapp, mariadb 컨테이너 기동 중.
- **2번 서버**: 빌드 서버. `deploy/build-and-push.sh`로 이미지 빌드 후 GHCR push.

배포 흐름: 2번에서 빌드 → GHCR push → 1번에서 `deploy/deploy.sh` 실행 (pull + recreate).
...
- 조회 API / QueryService / DTO
- Flyway 마이그레이션 V1 (스키마), V2 (샘플 데이터)
- 1번 서버에서 prod 프로파일로 앱 정상 기동

**미구현**
- Nginx 구성
- 키움 API 수집 파이프라인 (`external/`, `collector/`)
- 프론트엔드
- 텔레그램 발송 로직
...
- HTTPS: Duck DNS + Certbot (Let's Encrypt) 적용 예정.
