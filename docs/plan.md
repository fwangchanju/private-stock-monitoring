...
| 2 | 투자자별 매매종합 | ✅ 구현 완료 | ka10051 (1시간 주기, 13종 투자자) |
...
**수집 구조**
장중 1시간 주기: KRX 전종목 현재가·시가총액 수집 → 기여도 연산 → `IndexContributionRankingSnapshot` 저장
...
- 타임존: `Asia/Seoul` 명시
- `snapshotTime`: 현재 시각을 1시간 단위 내림 처리한 논리적 기준 시각
...
섹션 ② 참고. 수동 발송 버튼 구현됨, 자동 발송(스케줄 연동) 미연결.
...
### ② 텔레그램 이미지 발송 (`psms-renderer`)

**핵심 사이클**: 수집 완료 → 대시보드 캡처 → 텔레그램 이미지 발송

```
매 정시 (0 0 9-15 * * MON-FRI)
  1. Spring Boot: 데이터 수집 완료
  2. Spring Boot → psms-renderer POST /capture
  3. psms-renderer: Playwright로 대시보드 렌더링 → section별 이미지 반환
  4. Spring Boot: sendPhoto로 TELEGRAM_CHAT_ID에 발송
```

**psms-renderer 서비스** ✅ 구현 완료 (end-to-end 테스트 필요)
- Node.js + Express + Playwright 컨테이너 (`containers/renderer/`)
- `section.section` 요소 단위로 스크린샷, base64 응답
- 수동 발송 버튼 (`POST /api/send-dashboard`) 구현됨
- 자동 발송 (매 정시 스케줄 연동): 미연결
